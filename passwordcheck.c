	...
	switch (password_type)
	{
		...
		case PASSWORD_TYPE_PLAINTEXT:
			...
			if (pwdlen < MIN_PWD_LENGTH)
				ereport(ERROR,
						(errcode(ERRCODE_INVALID_PARAMETER_VALUE),
						 errmsg("password is too short. It must be at least 14 characters long")));
			...
			for (i = 0; i < pwdlen; i++)
			{
				...
				/*
				 * check for upper- and lowercase characters, as well as special characters
				 *
				 */
			...
			if (!pwd_has_letter || !pwd_has_nonletter || !pwd_has_upper || !pwd_has_lower || !pwd_has_special)
				ereport(ERROR,
						(errcode(ERRCODE_INVALID_PARAMETER_VALUE),
			errmsg("password must contain at least one uppercase, one lowercase letter, one digit, and one special character")));
